...
function initDashboardPage() {
    ...
    // LED Dashboard otomatik başlayacak (kendi script'i içinde)
}
...
async function getSingleFault(faultNo, timeout = 1000) { // Timeout'u düşür
    ...
    // SON 50 ARIZA FONKSIYONU - YENİ
    async function fetchLast50Faults() {
        if (isLoading) return;
        isLoading = true;
        
        const btnText = fetchLast50FaultsBtn.querySelector('.btn-text');
        const btnIcon = fetchLast50FaultsBtn.querySelector('.btn-icon');
        const btnLoader = fetchLast50FaultsBtn.querySelector('.btn-loader');
        const progressSection = document.getElementById('progressSection');
        
        fetchLast50FaultsBtn.disabled = true;
        btnIcon.style.display = 'none';
        btnLoader.style.display = 'inline-block';
        btnText.textContent = 'Sorgulanıyor...';
        ...
        try {
            ...
            // SON 50 KAYDINI AL
            const recordsToFetch = Math.min(50, totalCount);
            const startingFault = Math.max(1, totalCount - recordsToFetch + 1);
            
            showMessage(`✅ Son ${recordsToFetch} arıza kaydı alınıyor...`, 'info');
            
            updateProgress(0, recordsToFetch);
            updateElement('progressText', `Son ${recordsToFetch} arıza kaydı alınıyor...`);
            
            let successCount = 0;
            let failCount = 0;
            const startTime = Date.now();
            
            // Son 50 kaydı ters sırada al (en yeniden en eskiye)
            for (let i = totalCount; i >= startingFault; i--) {
                ...
                if (fault) {
                    ...
                    updateProgress(progressIndex, recordsToFetch);
                    updateElement('progressText', `Arıza ${progressIndex}/${recordsToFetch} alınıyor...`);
                    
                    // Her 10 kayıtta bir tabloyu güncelle
            ...
            updateProgress(recordsToFetch, recordsToFetch);
            updateElement('progressText', `İşlem tamamlandı!`);
            ...
            const elapsedTime = Math.round((Date.now() - startTime) / 1000);
            const summaryMsg = `✅ Son ${successCount} arıza kaydı ${elapsedTime} saniyede alındı` +
                              (failCount > 0 ? ` - ${failCount} başarısız` : '');
            showMessage(summaryMsg, 'success');
            ...
            console.error('Son 50 arıza sorgulama hatası:', error);
            showMessage('❌ ' + error.message, 'error');
            progressSection.style.display = 'none';
            
        } finally {
            isLoading = false;
            fetchLast50FaultsBtn.disabled = false;
            btnIcon.style.display = 'inline';
            btnLoader.style.display = 'none';
            btnIcon.textContent = '📋';
            btnText.textContent = 'Son 50 Arıza';
        }
    }
    
    // dsPIC'TEKİ ARIZALARI SİL - YENİ
    ...
    // Son 50 arıza butonu - YENİ
    if (fetchLast50FaultsBtn) {
        fetchLast50FaultsBtn.addEventListener('click', fetchLast50Faults);
    }
    
    // dsPIC arızalarını sil butonu - YENİ
