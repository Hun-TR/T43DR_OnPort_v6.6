...
function initLedPanelForDashboard() {
    ...
    function updateLedVisualsInDashboard(parsedData) {
        ...
        if (parsedData.inputs && Array.isArray(parsedData.inputs)) {
            for (let i = 0; i < 8; i++) {
                updateSingleLedInDashboard('led_I' + (i + 1), 'led_I' + (i + 1) + '_inner', parsedData.inputs[i], 'red');
            }
        }
        ...
        if (parsedData.outputs && Array.isArray(parsedData.outputs)) {
            for (let i = 0; i < 8; i++) {
                updateSingleLedInDashboard('led_O' + (i + 1), 'led_O' + (i + 1) + '_inner', parsedData.outputs[i], 'red');
            }
        }
        
        // Alarm LED'leri
        if (parsedData.alarms) {
            const hasAnyAlarm = parsedData.alarms.general || false;
            updateSingleLedInDashboard('led_B1', 'led_B1_inner', !hasAnyAlarm, 'green');
            updateSingleLedInDashboard('led_B2', 'led_B2_inner', parsedData.alarms.general || false, 'yellow');
            updateSingleLedInDashboard('led_B3', 'led_B3_inner', parsedData.alarms.ntp || false, 'yellow');
            updateSingleLedInDashboard('led_B4', 'led_B4_inner', parsedData.alarms.rs232 || false, 'yellow');
            updateSingleLedInDashboard('led_B5', 'led_B5_inner', parsedData.alarms.dc1 || false, 'yellow');
            updateSingleLedInDashboard('led_B6', 'led_B6_inner', parsedData.alarms.dc2 || false, 'yellow');
        }
    }
    
    function updateSingleLedInDashboard(ledId, ledInnerId, isOn, color) {
        ...
        if (isOn) {
            switch(color) {
                case 'green':
                    ...
                    break;
                case 'yellow':
                    ledOuter.setAttribute('fill', '#facc15');
                    ledOuter.setAttribute('stroke', '#eab308');
                    ledInner.setAttribute('fill', '#fef08a');
                    break;
                case 'red':
                default:
